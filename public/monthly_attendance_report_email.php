...
$sql_users = "SELECT id, name, email FROM users WHERE role_id != 1";
...
while ($user = $res_users->fetch_assoc()) {
    ...
    if ($attRes->num_rows == 0) {
        ...
        if ($leaveRes->num_rows > 0) {
            $table .= "<tr>
                ...
                <td colspan='4' style='padding:5px; border:1px solid #ddd; text-align:center; color:#3498db; font-weight:bold;'>Leave</td>
            </tr>";
        } else {
            $table .= "<tr>
                <td style='padding:5px; border:1px solid #ddd;'>{$user['name']}</td>
                <td colspan='4' style='padding:5px; border:1px solid #ddd; text-align:center; color:#e74c3c; font-weight:bold;'>Absent</td>
            </tr>";
        }
        continue;
    }
    ...
    $sqlMovements = "SELECT movement_type, movement_action, time FROM movements 
    ...
    if (!$resMovements) {
        $table .= "<tr>
            <td style='padding:5px; border:1px solid #ddd;'>{$user['name']}</td>
            <td colspan='4' style='padding:5px; border:1px solid #ddd; text-align:center;'>Error loading data</td>
        </tr>";
        continue;
    }
    ...
    foreach ($allDates as $date) {
        ...
        if (in_array($date, $leaveDates) && !isset($attendanceByDate[$date])) {
            ...
            $table .= "<tr>
                ...
                <td colspan='4' style='padding:4px; border:1px solid #ddd; text-align:center; color:#3498db; font-weight:bold;'>Leave</td>
            </tr>";
            continue;
        }
...
try {
    ...
    $res_users = $conn->query("SELECT email, name FROM users");
    ...
    $mail->send();
    echo "✅ Attendance summary email sent successfully!";
} catch (Exception $e) {
