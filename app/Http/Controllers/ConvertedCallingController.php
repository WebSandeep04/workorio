...
class ConvertedCallingController extends Controller
{
    public function index()
    {
        return view('calling.converted');
    }

    /**
     * Helper to get assigned query with campaign info
     */
    private function getConvertedQuery($userId)
    {
        ...
        return DB::table('calling_campaign_calling')
            ...
            ->select(
                ...
                DB::raw('(SELECT u.name FROM calling_assign_logs cal JOIN users u ON cal.assigned_to = u.id WHERE cal.calling_id = callings.id ORDER BY cal.id DESC LIMIT 1) as assigned_to_name')
            );
    }

    public function getConvertedCallings(Request $request)
    {
        $userId = $this->getCurrentUserId();
        $perPage = $request->get('per_page', 10);

        $query = $this->getConvertedQuery($userId);

        return response()->json($query->orderBy('calling_campaign_calling.id', 'desc')->paginate($perPage));
    }

    public function filterConvertedCallings(Request $request)
    {
        $userId = $this->getCurrentUserId();
        $perPage = $request->get('per_page', 10);
