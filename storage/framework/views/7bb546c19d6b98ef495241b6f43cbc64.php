...
<?php $__env->startSection('page_title', 'Subscription Status'); ?>
...
<div class="container-fluid px-2">
  <div class="table-search mb-2">
    <div class="table-search-field">
      ...
      <input type="text" id="search" placeholder="Search subscription statuses..." />
    </div>
    <button class="table-search-btn" data-bs-toggle="modal" data-bs-target="#createSubscriptionStatusModal">
  ...
  <div class="modern-card data-table-card">
    <div class="modern-card-body">
      <div class="table-responsive">
        <table class="table custom-table" id="subscriptionStatusTable">
          <thead>
            <tr>
              <th>Status Name</th>
          ...
          <tbody id="subscriptionStatusTableBody">
            <tr>
...
<div class="modal fade modal-modern" id="createSubscriptionStatusModal" tabindex="-1" aria-labelledby="createSubscriptionStatusModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="createSubscriptionStatusModalLabel">
          <i class="bi bi-plus-circle text-white"></i>
          Create Subscription Status
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="createSubscriptionStatusForm">
        <div class="modal-body pt-4 pb-4">
          <?php echo csrf_field(); ?>
          <div class="mb-3">
            <label for="subscription_status_name" class="form-label-modern">Status Name <span class="text-danger">*</span></label>
            <input type="text" class="form-control form-control-modern" id="subscription_status_name" name="status_name" required>
          </div>
        </div>
...
<div class="modal fade modal-modern" id="editSubscriptionStatusModal" tabindex="-1" aria-labelledby="editSubscriptionStatusModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="editSubscriptionStatusModalLabel">
          <i class="bi bi-pencil-square text-white"></i>
          Edit Subscription Status
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="editSubscriptionStatusForm">
        <div class="modal-body pt-4 pb-4">
          <?php echo csrf_field(); ?>
          <input type="hidden" id="edit_subscription_status_id">
          <div class="mb-3">
            <label for="edit_subscription_status_name" class="form-label-modern">Status Name <span class="text-danger">*</span></label>
            <input type="text" class="form-control form-control-modern" id="edit_subscription_status_name" required>
          </div>
        </div>
        <div class="modal-footer">
...
<div class="modal fade modal-modern" id="deleteStatusModal" tabindex="-1" aria-labelledby="deleteStatusModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body pt-4 pb-4">
        <p class="mb-0 text-center fs-6">Are you sure you want to delete this subscription status?</p>
      </div>
      <div class="modal-footer">
...
function buildSimplePagination($container, current, last) {
    console.log('Building pagination:', current, last);
    $container.empty();
    // Prev
    $container.append(`
...
$(function () {
  ...
  function loadSubscriptionStatuses(page = 1) {
    let search = $('#search').val();

    $('#subscriptionStatusTableBody').html(`
      <tr>
        <td colspan="2" class="loading-state">
          <i class="bi bi-arrow-repeat spin"></i>
          <p class="mt-2 mb-0">Loading statuses...</p>
    ...
    $.get(`<?php echo e(route('subscription-status.fetch')); ?>?page=${page}&search=${search}`)
      .done(function (data) {
          ...
          $('#subscriptionStatusTableBody').html(`
            <tr>
              <td colspan="2" class="empty-state">
                ...
                <p>Create your first subscription status to get started.</p>
              </td>
            </tr>
          `);
              ...
              <td>
                <div class="d-flex gap-2 justify-content-center">
                  <button 
                    ...
                    data-id="${status.id}"
                    data-name="${escapeHtml(status.status_name)}"
                    title="Edit"
                  >
        ...
        $('#subscriptionStatusTableBody').html(rows);
  ...
  $(document).on('click', '#paginationLinks .page-link', function (e) {
    ...
    if (page) {
      loadSubscriptionStatuses(page);
    }
  });

  $('#createSubscriptionStatusForm').submit(function (e) {
    e.preventDefault();
    const $btn = $('#createSubmitBtn');
    $btn.prop('disabled', true).html('<i class="bi bi-arrow-repeat spin"></i> Saving...');

    $.post("<?php echo e(route('subscription-status.store')); ?>", {
      status_name: $('#subscription_status_name').val(),
      _token: '<?php echo e(csrf_token()); ?>'
    })
      .done(function () {
        $('#createSubscriptionStatusModal').modal('hide');
        $('#createSubscriptionStatusForm')[0].reset();
        loadSubscriptionStatuses();
        showAlert('success', 'Subscription status created successfully.');
      })
      .fail(function (xhr) {
  ...
  $(document).on('click', '.editBtn', function () {
    $('#edit_subscription_status_id').val($(this).data('id'));
    $('#edit_subscription_status_name').val($(this).data('name'));
    $('#editSubscriptionStatusModal').modal('show');
  });

  $('#editSubscriptionStatusForm').submit(function (e) {
    e.preventDefault();
    const $btn = $('#editSubmitBtn');
    $btn.prop('disabled', true).html('<i class="bi bi-arrow-repeat spin"></i> Updating...');

    const id = $('#edit_subscription_status_id').val();
    $.ajax({
      url: `/subscription-status/${id}`,
      type: 'PUT',
      data: {
        status_name: $('#edit_subscription_status_name').val(),
        _token: '<?php echo e(csrf_token()); ?>'
      }
    })
      .done(function () {
        $('#editSubscriptionStatusModal').modal('hide');
        loadSubscriptionStatuses();
        showAlert('success', 'Subscription status updated successfully.');
      })
      .fail(function (xhr) {
         const errors = xhr.responseJSON?.errors;
  ...
  $('#confirmDeleteBtn').click(function() {
      ...
      $.ajax({
        ...
        .done(function () {
          ...
          loadSubscriptionStatuses();
          showAlert('success', 'Subscription status deleted successfully.');
        })
        .fail(function () {
          showAlert('error', 'Failed to delete subscription status.');
        })
        .always(function() {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\leadmanagement (akrati ui work)\resources\views/subscription-status.blade.php ENDPATH**/ ?>