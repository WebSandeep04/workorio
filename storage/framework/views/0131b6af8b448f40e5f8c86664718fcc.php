...
<?php $__env->startSection('title', 'Asset Statuses'); ?>
<?php $__env->startSection('page_title', 'Asset Statuses'); ?>
...
<div class="container-fluid px-2">
  <div class="table-search mb-2">
    <div class="table-search-field">
      ...
      <input type="text" id="search" placeholder="Search asset statuses..." />
    </div>
    <button class="table-search-btn" data-bs-toggle="modal" data-bs-target="#createAssetStatusModal">
  ...
  <div class="modern-card data-table-card">
    <div class="modern-card-body">
      <div class="table-responsive">
        <table class="table custom-table" id="assetStatusesTable">
          <thead>
            <tr>
              <th>Status Name</th>
              <th>Actions</th>
          ...
          <tbody>
            <tr>
              <td colspan="2" class="loading-state">
                ...
                <p class="mt-2 mb-0">Loading asset statuses...</p>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  <div class="table-range-meta" id="assetStatusRangeInfo">
...
<!-- Create Modal -->
<div class="modal fade modal-modern" id="createAssetStatusModal" tabindex="-1" aria-labelledby="createAssetStatusModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style ="font-size: 1.1rem; font-weight: 600;" id="createAssetStatusModalLabel">
      ...
      <form id="createAssetStatusForm">
        <div class="modal-body pt-4 pb-4">
          <?php echo csrf_field(); ?>
          <div class="mb-2">
            <label for="status_name" class="form-label-modern">Status Name <span class="text-danger">*</span></label>
            <input type="text" class="form-control form-control-modern" id="status_name" name="name" required placeholder="Enter status name">
          </div>
        </div>
...
<!-- Edit Modal -->
<div class="modal fade modal-modern" id="editAssetStatusModal" tabindex="-1" aria-labelledby="editAssetStatusModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style="font-size: 1.1rem; font-weight: 600;" id="editAssetStatusModalLabel">
          <i class="bi bi-pencil-square text-white"></i>
          Edit Asset Status
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="editAssetStatusForm">
        <div class="modal-body pt-4 pb-4">
          <?php echo csrf_field(); ?>
          <input type="hidden" id="edit_status_id">
          <div class="mb-2">
            <label for="edit_status_name" class="form-label-modern">Status Name <span class="text-danger">*</span></label>
...
function updateRangeInfo(from, to, total) {
    const $info = $('#assetStatusRangeInfo');
...
$(function () {
  ...
  function loadAssetStatuses(page = 1) {
    let search = $('#search').val();
    
    $('#assetStatusesTable tbody').html(`
      <tr>
        <td colspan="2" class="loading-state">
          <i class="bi bi-arrow-repeat spin"></i>
          <p class="mt-2 mb-0">Loading asset statuses...</p>
    ...
    $.get(`<?php echo e(route('asset-status.fetch')); ?>?page=${page}&search=${search}`, function (data) {
        ...
        $('#assetStatusesTable tbody').html(`
          <tr>
            <td colspan="2" class="empty-state">
              ...
              <p>Get started by creating your first asset status.</p>
            </td>
          </tr>
        `);
      ...
      let rows = '';
      $.each(data.data, function (i, status) {
        rows += `
            ...
            <td>
              <div class="d-flex gap-2 justify-content-center">
                <button class="btn-action btn-action-edit editBtn"
                  data-id="${status.id}" data-name="${status.name}" title="Edit">
      ...
      $('#assetStatusesTable tbody').html(rows);
  ...
  $('#createAssetStatusForm').submit(function (e) {
    e.preventDefault();
    const $btn = $(this).find('button[type="submit"]');
    $btn.prop('disabled', true).html('<i class="bi bi-arrow-repeat spin"></i> Creating...');
    
    $.post("<?php echo e(route('asset-status.store')); ?>", {
      name: $('#status_name').val(),
      _token: '<?php echo e(csrf_token()); ?>'
    }, function () {
      $('#createAssetStatusModal').modal('hide');
      $('#createAssetStatusForm')[0].reset();
      loadAssetStatuses();
      showAlert('success', 'Asset status created successfully.');
    }).fail(function (xhr) {
        let msg = 'Failed to create asset status.';
  ...
  $(document).on('click', '.editBtn', function () {
    ...
    $('#editAssetStatusModal').modal('show');
  });

  $('#editAssetStatusForm').submit(function (e) {
    e.preventDefault();
    const $btn = $(this).find('button[type="submit"]');
    ...
    $.ajax({
      ...
      success: function () {
        $('#editAssetStatusModal').modal('hide');
        loadAssetStatuses();
        showAlert('success', 'Asset status updated successfully.');
      },
      error: function(xhr) {
        let msg = 'Failed to update asset status.';
        if(xhr.responseJSON && xhr.responseJSON.message) msg = xhr.responseJSON.message;
        if(xhr.responseJSON && xhr.responseJSON.errors) msg = Object.values(xhr.responseJSON.errors).join("\\n");
        alert(msg);
      },
  ...
  $(document).on('click', '.deleteBtn', function () {
    if (confirm('Are you sure you want to delete this asset status?')) {
      $.ajax({
        url: `/asset-status/${$(this).data('id')}`,
        type: 'DELETE',
        data: { _token: '<?php echo e(csrf_token()); ?>' },
        success: function () {
          loadAssetStatuses();
          showAlert('success', 'Asset status deleted successfully.');
        },
        error: function() {
          showAlert('error', 'Failed to delete asset status.');
        }
      });
    }
  });
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\DontDelete\laravel\leadmanagement (akrati ui work)\resources\views/software-setup/asset-status/index.blade.php ENDPATH**/ ?>