...
<?php $__env->startSection('page_title', 'Calendar'); ?>
...
(function(){
    ...
    function loadEvents(year, month){
        ...
        const to = ymd(last);
        return $.get("<?php echo e(route('calendar.grid')); ?>", { from, to }).then(function(data){
            eventsByDate = {};
    ...
    $(document).on('click', '.day-cell', function(){
        ...
        $.get(`<?php echo e(url('/calendar/date')); ?>/${date}/handles`).done(function(resp){
    ...
    $(document).on('change', '.date-handle-checkbox', function(){
        ...
        $.ajax({
            url: '<?php echo e(route("calendar.date.handle.toggle")); ?>',
            method: 'POST',
            data: {
    ...
    $(document).on('change', '.date-client-status', function(){
        ...
        if (statusName === 'missed') {
            ...
            $.get("<?php echo e(route('calendar-missed-reasons.fetch')); ?>").done(function(missedReasons){
        ...
        $.get(`<?php echo e(url('/calendar/status')); ?>/${statusId}/checklists`).done(function(resp){
            if (!resp) return;
    ...
    function saveClientStatus(clientId, statusId, checklistOptionIds, missedReasonId, descriptions){
        ...
        $.ajax({
            url: '<?php echo e(route("calendar.date.client.status")); ?>',
            method: 'POST',
            data: {
                date: currentModalDate,
                client_id: clientId,
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\leadmanagement (akrati ui work)\resources\views/calendar/index.blade.php ENDPATH**/ ?>