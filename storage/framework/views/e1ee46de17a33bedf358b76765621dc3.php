    ...
    function loadTasks() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                const inProgress = data.filter(t => !t.is_done && t.status && (t.status.name === 'In Progress' || t.status.name === 'In progress')).length;
                const completed = data.filter(t => t.is_done || (t.status && t.status.name === 'Completed')).length;
                const pending = data.filter(t => !t.is_done && (!t.status || t.status.name === 'Pending')).length;
                ...
                $.get("<?php echo e(route('task.statuses')); ?>", function(statuses) {
                    ...
                    statuses.forEach(function(status) {
                        const statusTasks = data.filter(function(task) {
                            return task.task_status_id == status.id && (!task.is_done);
                        });
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\Don't Delete\laravel\leadmanagement (akrati ui work)\resources\views/worklog/my-tasks.blade.php ENDPATH**/ ?>