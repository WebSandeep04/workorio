      ...
      <div class="modal-body p-4">
        ...
        <div class="row g-3 mb-4">
            ...
            <div class="col-md-6">
             ...
             <div class="col-md-6">
                 <div class="d-flex align-items-center">
                    <div class="icon-box me-3 bg-light text-warning rounded-circle d-flex align-items-center justify-content-center" style="width: 40px; height: 40px;">
                        <i class="bi bi-kanban fs-5"></i>
                    </div>
                    <div>
                        <small class="text-muted text-uppercase fw-bold" style="font-size: 0.7rem;">Project</small>
                        <p id="view_customer_project" class="mb-0 fw-semibold text-dark"></p>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="modal-body form-compact">
                    ...
                    <div class="row">
                        <div class="mb-3 col-md-6">
                            ...
                            <div class="mt-2">
                                <label for="edit_customer_project_id" class="form-label">Project (Optional)</label>
                                <select name="customer_project_id" id="edit_customer_project_id" class="form-select form-select-sm">
                                    <option value="">Select Project</option>
                                </select>
                             </div>
                        </div>
...
$(document).ready(function() {
    ...
    // Event listener for Edit Task Customer change
    $('#edit_customer_id').on('change', function() {
        loadCustomerProjects($(this).val(), '#edit_customer_project_id');
    });
    ...
    function loadSummaryStats() {
      ...
      const inProgress = allTasks.filter(t => {
        ...
        return !isTaskDone && (statusName.includes('progress') || statusName.includes('ongoing'));
      }).length;
      ...
      const pending = allTasks.filter(t => {
        const statusName = (t.status?.name || '').toLowerCase();
        const isTaskDone = t.is_done || statusName === 'done' || statusName.includes('completed');
        return !isTaskDone && (statusName.includes('pending') || !statusName);
      }).length;
    ...
    window.viewTaskDetails = function(id) {
      ...
      if (task) {
        ...
        $('#view_customer_project').text(task.customer_project ? task.customer_project.project_name : 'N/A');
                    ...
                    $.each(data, function(index, task) {
                        ...
                        html += `
                            <tr class="${task.is_done ? 'table-success' : ''}">
                                <td><input type="checkbox" class="task-checkbox form-check-input" value="${task.id}"></td>
                                <td>
                                ...
                                <td>
                                    ...
                                    <button class="btn btn-sm btn-outline-secondary position-relative" onclick="showRemarksModal(${task.id})" title="Remarks">
                                        <i class="bi bi-chat-left-text"></i>
                                        ${(task.remarks ? task.remarks.length : 0) > 0 ? `<span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" style="font-size:0.55rem; padding: 2px 4px;">${task.remarks.length}</span>` : ''}
                                    </button>
        ...
        $.get("<?php echo e(route('task.fetch')); ?>", function(data) {
            ...
            if (task) {
                ...
                setTimeout(function() {
                    ...
                    // Trigger change to load projects, then set value
                    loadCustomerProjects(task.customer_id, '#edit_customer_project_id', task.customer_project_id);
