...
<style>
  ...
  /* Task Type Wrapper */
    ...
    .summary-card { background: #fff; border-radius: 10px; border: 1px solid #eceef3; padding: 0.75rem; box-shadow: 0px 4px 4px 0px #0000000A; transition: all 0.3s ease; width: 100%; min-height: 70px; height: auto; display: flex; align-items: center; gap: 0.75rem; }
    ...
    .summary-card-icon img { width: 22px; height: 22px; object-fit: contain; }
            ...
            <table class="custom-table" id="projectTasksTable">
                <thead>
                    <tr>
                        ...
                        <th style="width: 15%;">Est Efforts</th>
                    ...
                    <div class="p-3 form-compact flex-grow-1 overflow-auto">
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-3">
                                <label for="estimated_efforts" class="form-label">Estimated Efforts</label>
                                <input type="text" name="estimated_efforts" id="task_estimated_efforts" class="form-control form-control-sm" placeholder="e.g. 2h, 3 days...">
                            </div>
...
$(document).ready(function() {
    ...
    $('#createTaskForm').on('submit', function(e) {
        ...
        formData.set('estimated_efforts', $('#task_estimated_efforts').val() || '');
        
        // Append checked user_ids manually to be completely safe
        formData.delete('user_ids[]');
        $('#assignUsersGrid input[name="user_ids[]"]:checked').each(function() {
            formData.append('user_ids[]', $(this).val());
        });
        ...
        $.ajax({
             ...
             error: function(xhr) {
                 alert('Error creating task: ' + (xhr.responseJSON?.message || xhr.responseText));
             }
        });
    });
    ...
    function fetchProjectTasks() {
        $('#projectTasksTable tbody').html('<tr><td colspan="11" class="text-center p-4"><div class="spinner-border text-primary" role="status"></div><div class="mt-2">Loading tasks...</div></td></tr>');
        ...
        $.ajax({
            ...
            error: function(xhr) {
                $('#projectTasksTable tbody').html('<tr><td colspan="11" class="text-center text-danger p-4">Failed to load tasks.</td></tr>');
            }
        });
    }
    ...
    function renderTasksTable(tasks) {
        let html = '';
        if(tasks.length === 0) {
            html = '<tr><td colspan="11" class="text-center p-4 text-muted">No tasks found for this project.</td></tr>';
        } else {
            tasks.forEach(task => {
                 // Check Overdue
                let isOverdue = false;
                ...
                html += `
                    <tr class="${rowClass}">
                        ...
                        <td>
                            <span class="d-block text-truncate" title="${task.estimated_efforts || 'N/A'}">
    ...
    $('#editTaskForm').on('submit', function(e) {
        ...
        // Append checked user_ids manually to be completely safe
        formData.delete('user_ids[]');
        $('#editAssignUsersGrid input[name="user_ids[]"]:checked').each(function() {
            formData.append('user_ids[]', $(this).val());
        });
