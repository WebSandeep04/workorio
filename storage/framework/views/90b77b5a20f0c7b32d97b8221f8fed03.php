...
<?php $__env->startSection('page_title', 'Task Status'); ?>
...
<div class="container-fluid px-2">
  <div class="table-search mb-2">
    <div class="table-search-field">
      ...
      <input type="text" id="search" placeholder="Search task statuses..." />
    </div>
    ...
    <div class="modal-content">
      ...
      <form id="createStatusForm">
        <div class="modal-body pt-4 pb-4">
          ...
          <div class="mb-3">
            <label for="status_color" class="form-label-modern">Color</label>
            <input type="color" class="form-control form-control-color w-100" id="status_color" value="#6c757d" style="height: 48px;">
          </div>
          <div class="mb-3">
            <label for="status_order" class="form-label-modern">Order</label>
            <input type="number" class="form-control form-control-modern" id="status_order" value="0" min="0">
          </div>
        </div>
    ...
    <div class="modal-content">
      ...
      <form id="editStatusForm">
        <div class="modal-body pt-4 pb-4">
          ...
          <div class="mb-3">
            <label for="edit_status_color" class="form-label-modern">Color</label>
            <input type="color" class="form-control form-control-color w-100" id="edit_status_color" style="height: 48px;">
          </div>
          <div class="mb-3">
            <label for="edit_status_order" class="form-label-modern">Order</label>
            <input type="number" class="form-control form-control-modern" id="edit_status_order" min="0">
          </div>
        </div>
        <div class="modal-footer">
...
<div class="modal fade modal-modern" id="deleteStatusModal" tabindex="-1" aria-labelledby="deleteStatusModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      ...
      <div class="modal-body pt-4 pb-4">
        <p class="mb-0 text-center fs-6">Are you sure you want to delete this task status? <br><span class="text-danger small">This action cannot be undone if it is being used.</span></p>
      </div>
      <div class="modal-footer">
...
$(function () {
  ...
  function loadStatuses(page = 1) {
    ...
    $.get(`<?php echo e(route('task-status.fetch')); ?>?page=${page}&search=${search}`)
      .done(function (data) {
          ...
          $('#statusTableBody').html(`
            <tr>
              <td colspan="4" class="empty-state">
                ...
                <h5>No Task Statuses Found</h5>
                <p>Create your first task status to get started.</p>
              </td>
            </tr>
          `);
        ...
        $('#statusTableBody').html(`
          <tr>
            <td colspan="4" class="text-danger text-center py-4">
              ...
              Failed to load task statuses. Please try again.
            </td>
          </tr>
        `);
      });
  ...
  $('#createStatusForm').on('submit', function (e) {
    ...
    $.post("<?php echo e(route('task-status.store')); ?>", {
      ...
      .done(function (response) {
        ...
        loadStatuses();
        showAlert('success', response.message || 'Task status created successfully.');
      })
      .fail(function (xhr) {
        const errors = xhr.responseJSON?.errors;
        const message = errors ? Object.values(errors).flat().join(', ') : (xhr.responseJSON?.message || 'Failed to create task status.');
  ...
  $(document).on('click', '.editBtn', function () {
    ...
    $('#edit_status_color').val($(this).data('color'));
    $('#edit_status_order').val($(this).data('order'));
  ...
  $('#editStatusForm').on('submit', function (e) {
    ...
    $.ajax({
      ...
      .done(function (response) {
        ...
        loadStatuses();
        showAlert('success', response.message || 'Task status updated successfully.');
      })
      .fail(function (xhr) {
        const errors = xhr.responseJSON?.errors;
        const message = errors ? Object.values(errors).flat().join(', ') : (xhr.responseJSON?.message || 'Failed to update task status.');
        showAlert('error', message);
      })
      .always(function () {
  ...
  $('#confirmDeleteBtn').click(function() {
      ...
      $.ajax({
        ...
        .done(function (response) {
          ...
          loadStatuses();
          showAlert('success', response.message || 'Task status deleted successfully.');
        })
        .fail(function (xhr) {
          const message = xhr.responseJSON?.message || 'Failed to delete task status.';
          showAlert('error', message);
        })
        .always(function() {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\leadmanagement (akrati ui work)\resources\views/task-status/index.blade.php ENDPATH**/ ?>