...
<script>
    $(document).ready(function () {
        ...
        function loadCitiesByState(stateId) {
            ...
            const url = '<?php echo e(route("calling.assigned.cities", ["stateId" => 0])); ?>'.replace(/0$/, String(stateId));
        ...
        function loadAssignedCallings(page = 1) {
            currentPage = page;
            $.get('<?php echo e(route("calling.assigned.data")); ?>?page=' + page, function (data) {
        ...
        function applyFilters(page = 1) {
            ...
            $.post('<?php echo e(route("calling.assigned.filter")); ?>?page=' + page, { name, state_id: stateId, city_id: cityId, calling_type_id: callingTypeId })
                .done(function (data) {
                    const rows = Array.isArray(data) ? data : (data.data || []);
                    renderRows(rows);
        ...
        $(document).on('change', '.calling-type-select', function () {
            ...
            $.ajax({
                url: '<?php echo e(route("calling.assigned.update-type")); ?>',
                type: 'POST',
                data: {
