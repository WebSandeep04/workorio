...
(function () {
  ...
  function renderTableRows(rows) {
    if (!rows.length) {
      return '<tr><td colspan="13" class="text-center py-4 text-muted">No records found.</td></tr>';
    }
        ...
        <tr>
          ...
          <td>
            <div class="btn-group">
              ...
              <button class="btn btn-sm text-info bg-white im-followup-btn" style="border:1px solid #434AFA;" data-lead-id="${r.id}" title="Follow Up"><i class="bi bi-chat-text" style="color:#434AFA !important;"></i> Remark</button>
            </div>
          </td>
        </tr>
      `;
                ...
                html += `
                    <div class="comment-item p-3 bg-white rounded shadow-sm border border-light">
                        <div class="d-flex justify-content-between align-items-center mb-1">
                             <div class="comment-date text-muted small fw-bold">${c.created_at ? new Date(c.created_at).toLocaleString('en-IN') : 'N/A'} ${c.user ? ' - ' + c.user.name : ''}</div>
                        </div>
