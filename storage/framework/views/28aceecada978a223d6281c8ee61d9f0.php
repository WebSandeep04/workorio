...
<?php $__env->startSection('title', 'Payment Followup'); ?>
<?php $__env->startSection('page_title', 'Payment Followup'); ?>
...
function loadStats() {
    $.ajax({
        url: '<?php echo e(route("payment-followup.stats")); ?>',
        type: 'GET',
        data: getFilterData(),
...
function openDetails(type) {
    ...
    // Construct URL
    const url = '<?php echo e(route("payment-followup.details", ["type" => ":type"])); ?>'.replace(':type', type) + '?' + params.toString();
...
function loadPaymentFollowupLeads(page = 1) {
    ...
    $.ajax({
        url: '<?php echo e(route("payment-followup.filter")); ?>?page=' + page,
        type: 'POST',
        data: data,
        success: function (data) {
            let html = '';
...
function openCustomerDetails(id, name) {
    ...
    const url = '<?php echo e(route("payment-followup.details", ["type" => "total-invoices"])); ?>?customer_id=' + id;
    window.location.href = url;
}
...
$(document).ready(function () {
    ...
    // Load filter options
    $.ajax({
        url: '<?php echo e(route("payment-followup.filter-options")); ?>',
        type: 'GET',
        success: function(data) {
            // Load states
    ...
    $('#state').on('change', function() {
        ...
        if (stateId) {
            $.ajax({
                url: `<?php echo e(route("payment-followup.cities", ":id")); ?>`.replace(':id', stateId),
                type: 'GET',
...
$('#addInvoiceModal').on('show.bs.modal', function () {
    ...
    $.ajax({
        url: '<?php echo e(route("payment-followup.customers")); ?>',
        type: 'GET',
        success: function(data) {
    ...
    // Load products
    $.ajax({
        url: '<?php echo e(route("payment-followup.products")); ?>',
        type: 'GET',
        success: function(data) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\DontDelete\laravel\leadmanagement (akrati ui work)\resources\views/payment-followup.blade.php ENDPATH**/ ?>