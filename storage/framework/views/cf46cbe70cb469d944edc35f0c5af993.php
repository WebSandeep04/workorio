...
<?php
    ...
    $htmlLines = [];
    $htmlLines[] = '<form method="POST" action="' . route('formbuilder.submit', $form->id) . '" class="row g-3">';
    $htmlLines[] = '    @csrf';
    ...
    foreach ($fields as $field) {
        ...
        $htmlLines[] = '    <div class="col-12">';
        $htmlLines[] = '        <label class="form-label">' . $safeLabel . '</label>';
        if ($type === 'textarea') {
            $htmlLines[] = '        <textarea class="form-control" name="' . $safeName . '" rows="3" placeholder="Enter ' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '"' . $requiredAttr . '></textarea>';
        } else {
            $htmlLines[] = '        <input class="form-control" type="' . $type . '" name="' . $safeName . '" placeholder="Enter ' . htmlspecialchars($label, ENT_QUOTES, 'UTF-8') . '"' . $requiredAttr . ' />';
        }
        $htmlLines[] = '    </div>';
    }
    $htmlLines[] = '    <div class="col-12 text-end">';
    $htmlLines[] = '        <button type="submit" class="btn btn-primary">Submit</button>';
    $htmlLines[] = '    </div>';
    $htmlLines[] = '</form>';
    $htmlSnippet = implode("\n", $htmlLines);
?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\leadmanagement (akrati ui work)\resources\views/formbuilder/show.blade.php ENDPATH**/ ?>