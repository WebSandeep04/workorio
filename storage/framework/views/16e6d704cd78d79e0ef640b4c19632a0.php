...
<?php $__env->startSection('page_title', 'Opening Balance'); ?>
...
<div class="container-fluid px-2">
  <div class="table-search mb-2">
    <div class="table-search-field">
      ...
      <input type="text" id="search" placeholder="Search balances..." />
    </div>
    <button class="table-search-btn" data-bs-toggle="modal" data-bs-target="#createBalanceModal">
      <i class="bi bi-plus me-1"></i>Add Opening Balance
    </button>
  </div>
  ...
  <div class="modern-card data-table-card">
    <div class="modern-card-body">
      <div class="table-responsive">
        <table class="table custom-table" id="balanceTable">
          <thead>
            <tr>
              <th>Date Created</th>
              <th>Department</th>
          ...
          <tbody>
            <tr>
              <td colspan="4" class="loading-state">
                ...
                <p class="mt-2 mb-0">Loading balances...</p>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  <div class="table-range-meta" id="balanceRangeInfo">
...
<div class="modal fade modal-modern" id="createBalanceModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style ="font-size: 1.1rem; font-weight: 600;" id="createBalanceModalLabel">
          <i class="bi bi-plus text-white"></i>
          Add Opening Balance
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="createBalanceForm">
        <div class="modal-body pt-4 pb-4">
          <?php echo csrf_field(); ?>
          <div class="mb-3">
            <label class="form-label-modern">Department <span class="text-danger">*</span></label>
            <select class="form-select form-control-modern" name="department_id" required>
          ...
          <div class="mb-3">
            ...
            <input type="number" step="0.01" class="form-control form-control-modern" name="amount" required placeholder="0.00">
          </div>
        </div>
...
<!-- Edit Balance Modal -->
<div class="modal fade modal-modern" id="editBalanceModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style ="font-size: 1.1rem; font-weight: 600;" id="editBalanceModalLabel">
      ...
      <form id="editBalanceForm">
        <div class="modal-body pt-4 pb-4">
          ...
          <div class="mb-3">
            <label class="form-label-modern">Department <span class="text-danger">*</span></label>
            <select class="form-select form-control-modern" id="edit_department_id" name="department_id" required>
              <option value="">Select Department</option>
              <?php $__currentLoopData = $departments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $department): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <option value="<?php echo e($department->id); ?>"><?php echo e($department->name); ?></option>
              <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
          ...
          <div class="mb-3">
            <label class="form-label-modern">Amount <span class="text-danger">*</span></label>
            <input type="number" step="0.01" class="form-control form-control-modern" id="edit_amount" name="amount" required>
          </div>
        </div>
        <div class="modal-footer">
          <button type="submit" class="btn-modern btn-modern-primary w-100 justify-content-center" style="background: #434AFA; color: white;">
...
<div class="modal fade modal-modern" id="deleteBalanceModal" tabindex="-1" aria-labelledby="deleteBalanceModalLabel" aria-hidden="true" data-bs-backdrop="static">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" style ="font-size: 1.1rem; font-weight: 600;" id="deleteBalanceModalLabel">
      ...
      <div class="modal-body pt-4 pb-4">
        <p class="mb-0 text-center fs-6">Are you sure you want to delete this specific balance entry?</p>
      </div>
      <div class="modal-footer">
...
$(function () {
  ...
  function loadData(page = 1) {
    ...
    $.get(`<?php echo e(route('petty-opening-balance.fetch')); ?>?page=${page}&search=${search}`, function (data) {
        ...
        $('#balanceTable tbody').html(`
          <tr>
            <td colspan="4" class="empty-state">
              ...
              <p>Get started by adding an opening balance.</p>
            </td>
          </tr>
        `);
          ...
          <tr style="animation-delay: ${i * 0.1}s;">
            ...
            <td>
              <div class="d-flex gap-2 justify-content-center">
                ...
                <button class="btn-action btn-action-delete deleteBtn" data-id="${item.id}" title="Delete">
                  <i class="bi bi-trash"></i>
                </button>
              </div>
            </td>
      ...
      $('#balanceTable tbody').html(`
        <tr>
          <td colspan="4" class="text-danger text-center py-4">
            ...
            <p class="mt-2 mb-0">Failed to load balances. Please try again.</p>
          </td>
        </tr>
      `);
    });
  ...
  $('#createBalanceForm').submit(function (e) {
    e.preventDefault();
    const $btn = $(this).find('button[type="submit"]');
    $btn.prop('disabled', true).html('<i class="bi bi-arrow-repeat spin"></i> Creating...');
    
    $.post("<?php echo e(route('petty-opening-balance.store')); ?>", $(this).serialize(), function () {
      $('#createBalanceModal').modal('hide');
      $('#createBalanceForm')[0].reset();
      loadData();
      showAlert('success', 'Opening balance added successfully.');
    }).fail(function (xhr) {
  ...
  $('#editBalanceForm').submit(function (e) {
    ...
    let id = $('#edit_id').val();
    $.ajax({
      url: `/petty-opening-balance/${id}`,
      type: 'PUT',
      data: $(this).serialize(),
      success: function () {
        $('#editBalanceModal').modal('hide');
        loadData();
        showAlert('success', 'Opening balance updated successfully.');
      },
      error: function(xhr) {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laravel\leadmanagement (akrati ui work)\resources\views/opening-balance.blade.php ENDPATH**/ ?>