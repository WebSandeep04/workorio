...
@section('title', 'Calendar Social Handles')
@section('page_title', 'Calendar Social Handles')
...
$(function () {
  ...
  function loadSocial(page = 1) {
    ...
    $.get(`{{ route('calendar-social.fetch') }}?page=${page}&search=${search}`)
      .done(function (data) {
  ...
  $('#createSocialForm').on('submit', function (e) {
    ...
    $.post("{{ route('calendar-social.store') }}", {
