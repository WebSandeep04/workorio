...
@section('title', 'Loan Installments Schedule')
@section('page_title', 'Loan Installments Schedule')
...
<div class="container-fluid px-2 mt-2">
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
        <a href="{{ route('loans.manage') }}" class="btn btn-secondary btn-sm"><i class="bi bi-arrow-left"></i> Back to Manage Loans</a>
    </div>
    ...
    <div class="summary-cards">
        <div class="summary-card">
            ...
            <div class="summary-card-content">
                ...
                <div class="summary-card-value">{{ number_format($loan->amount, 2) }}</div>
            </div>
        </div>
        <div class="summary-card">
            ...
            <div class="summary-card-content">
                ...
                <div class="summary-card-value">{{ $loan->total_installments }}</div>
            </div>
        </div>
        <div class="summary-card">
            ...
            <div class="summary-card-content">
                ...
                <div class="summary-card-value">{{ number_format($loan->installments->where('status', 'paid')->sum('amount'), 2) }}</div>
            </div>
        </div>
        <div class="summary-card">
            ...
            <div class="summary-card-content">
                ...
                <div class="summary-card-value">{{ number_format($loan->installments->where('status', 'skipped')->sum('amount'), 2) }}</div>
            </div>
        </div>
        <div class="summary-card">
            ...
            <div class="summary-card-content">
                ...
                <div class="summary-card-value">{{ number_format($loan->remaining_balance, 2) }}</div>
            </div>
        </div>
        <div class="summary-card">
            <div class="summary-card-icon {{ $loan->status === 'approved' ? 'icon-green' : ($loan->status === 'rejected' ? 'icon-red' : 'icon-orange') }}">
                <i class="bi bi-info-circle"></i>
            </div>
            <div class="summary-card-content">
                <div class="summary-card-label">Status</div>
                <div class="summary-card-value">
                    <span class="badge {{ $loan->status === 'approved' ? 'bg-success' : ($loan->status === 'rejected' ? 'bg-danger' : 'bg-warning') }}" style="font-size: 0.8rem;">
                        {{ ucfirst($loan->status) }}
                    </span>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($loan->installments as $inst)
                            ...
                            <tr>
                                ...
                                <td>{{ strtolower(\Carbon\Carbon::parse($inst->due_month)->format('M-Y')) }}</td>
                                <td>{{ number_format($inst->amount, 2) }}</td>
                                <td><span class="{{ $iBadge }}">{{ ucfirst($inst->status) }}</span></td>
                                <td>
                                    @if($inst->status === 'pending' && $inst->due_month === $currentMonth)
                                        <button type="button" class="btn-skip open-skip-modal" data-id="{{ $inst->id }}">Skip</button>
                                    @else
