...
<style>
    ...
    .badge-junk { background: #fee2e2; color: #ef4444; font-size: 10px; padding: 2px 8px; border-radius: 20px; font-weight: 700; }
...
<div class="container-fluid px-2 calling-page">
    ...
    <div class="filterBox">
        <div>
            <label for="filter_state" class="form-label-modern">State</label>
            <select id="filter_state" class="form-control-modern"><option value="">All States</option></select>
        </div>
        <div>
            <label for="filter_city" class="form-label-modern">City</label>
    ...
    <div class="data-table-card">
        <div class="table-scroll">
            <table class="table custom-table" id="junkTable">
                <thead>
                    <tr>
                        <th>Campaign</th>
                        <th>Lead Name</th>
...
<script>
    $(document).ready(function() {
            ...
            if (rows && rows.length) {
                rows.forEach(function(r){
                    ...
                    html += `<tr>
                        ...
                        <td>${r.city || '-'}</td>
                        <td>${r.phone || '-'}</td>
                        <td><span class="badge-junk">JUNK</span></td>
                        <td>
                            <div class="d-flex gap-2">
                                <button class="btn btn-sm btn-outline-success border-0 px-2" onclick="performRestore(${r.pivot_id})" title="Restore Lead">
                                    <i class="bi bi-arrow-counterclockwise"></i>
                                </button>
                                <button class="btn btn-sm btn-outline-danger border-0 px-2" onclick="performDelete(${r.pivot_id})" title="Purge Record">
                ...
                html = '<tr><td colspan="9" class="text-center p-5 text-muted">Junk pool is empty. Good job!</td></tr>';
            }
            $tbody.html(html);
        }

        window.performRestore = function(pivotId) {
            Swal.fire({
                title: 'Restore Lead?',
                text: "Move this lead back to active status?",
                icon: 'question',
        ...
        window.performDelete = function(pivotId) {
            Swal.fire({
                title: 'Purge Record?',
                text: "This will permanently remove the lead and all its remarks!",
                icon: 'warning',
                showCancelButton: true,
        ...
        function loadData(page = 1) {
            ...
            .done(function(data){
                ...
                $('#totalJunk').text((data.total || 0).toLocaleString('en-IN'));
