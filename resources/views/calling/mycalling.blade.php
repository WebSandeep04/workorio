...
<script>
    $(document).ready(function () {
        ...
        function loadCitiesByState(stateId) {
            ...
            const url = '{{ route("calling.my.cities", ["stateId" => 0]) }}'.replace(/0$/, String(stateId));
        ...
        function applyFilters(page = 1) {
            ...
            $.post('{{ route("calling.my.filter") }}?page=' + page, { name, state_id: stateId, city_id: cityId, calling_type_id: callingTypeId })
                .done(function (data) {
                    const rows = Array.isArray(data) ? data : (data.data || []);
                    renderRows(rows);
        ...
        $(document).on('change', '.calling-type-select', function () {
            ...
            $.ajax({
                url: '{{ route("calling.my.update-type") }}',
                type: 'POST',
                data: {
