...
$(function () {
    ...
    function loadFilterOptions() {
        return $.get('{{ route("calling.all.filter-options") }}').done((resp) => {
    ...
    function loadCities(stateId) {
        ...
        const url = '{{ route("calling.all.cities", ["stateId" => 0]) }}'.replace(/0$/, String(stateId));
        $.get(url).done((cities) => {
    ...
    function renderRows(rows) {
        ...
        if (rows && rows.length) {
            rows.forEach((r) => {
                ...
                const callTypeColor = cTypeId === 9 ? '#10b981' : (cTypeId === 7 ? '#8b5cf6' : '#6366f1');
                const typeHtml = `<span style="display:inline-flex; align-items:center; px-2 py-1; font-size:0.75rem; font-weight:600; color:${callTypeColor}; background:${callTypeColor}15; border-radius:0.375rem; border:1px solid ${callTypeColor}30; padding:0.25rem 0.5rem;"><i class="bi bi-circle-fill me-1" style="font-size:0.4rem;"></i> ${cTypeName}</span>`;
            ...
            html = '<tr><td colspan="10" class="text-center">No calls found.</td></tr>';
        }
        $tbody.html(html);
    }
    ...
    function loadCallings(page = 1) {
        currentPage = page;
        $.get('{{ route("calling.all.data") }}?page=' + page).done((data) => {
    ...
    function applyFilters(page = 1) {
        ...
        $.post('{{ route("calling.all.filter") }}?page=' + page, {
            name: name,
            state_id: stateId,
            city_id: cityId,
