...
<script>
    $(document).ready(function () {
        ...
        function loadCitiesByState(stateId) {
            ...
            const url = '{{ route("calling.team.cities", ["stateId" => 0]) }}'.replace(/0$/, String(stateId));
        ...
        function loadTeamCallings(page = 1) {
            ...
            $.get('{{ route("calling.team.data") }}?page=' + page, function (data) {
        ...
        function applyFilters(page = 1) {
            ...
            $.post('{{ route("calling.team.filter") }}?page=' + page, { name, state_id: stateId, city_id: cityId, calling_type_id: callingTypeId })
                .done(function (data) {
                    const rows = Array.isArray(data) ? data : (data.data || []);
                    renderRows(rows);
        ...
        $(document).on('change', '.calling-type-select', function () {
            ...
            $.ajax({
                url: '{{ route("calling.team.update-type") }}',
                type: 'POST',
                data: {
        ...
        window.reassignCalling = function(callingId, newUserId) {
            if (!newUserId) return;
            $.ajax({
                url: '{{ route("calling.team.reassign") }}',
                type: 'POST',
                data: {
                    calling_id: callingId,
        ...
        function loadTeamMembers() {
            return new Promise(function(resolve, reject) {
                $.ajax({
                    url: '{{ route("calling.team.team-members") }}',
                    type: 'GET',
                    success: function (response) {
