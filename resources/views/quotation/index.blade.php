...
<div class="container-fluid px-2">
    ...
    <div class="modern-card">
        <div class="modern-card-header">
            ...
            <a href="{{ route('quotation.create') }}" class="btn-add-modern">
                Add +
            </a>
        </div>
...
function reviseQuotation(id, quotationNo) {
    ...
    window.location.href = `{{ route('quotation.create') }}?quote=${encodeURIComponent(quotationNo)}&revise=1`;
}
...
function openRevisionHistory(id, no){
    ...
    let url = `{{ url('/quotation') }}/${id}/revisions`;
    $.get(url)
      .done(function(resp){
        const rows = (resp && resp.data) ? resp.data : [];
        if(!rows.length){
