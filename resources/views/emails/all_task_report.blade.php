...
<div style="width: 100%; max-width: 1400px; margin: 0 auto; background-color: #ffffff; padding: 0; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
    ...
    <div style="padding:15px 2px 25px 2px;">
        @foreach ($payload['tasksByUser'] as $uid => $ud)
            <div style="margin-bottom:20px;">
                <div style="background-color: #6c757d; color:white; padding:8px 12px;">
                    <h3 style="margin:0; font-size:13px; font-weight:600;">
                        👤 {{ $ud['user_name'] }}
                        <span style="background-color:rgba(255,255,255,0.2); padding:2px 6px; border-radius:8px; font-size:10px; margin-left:8px;">{{ count($ud['tasks']) }} tasks</span>
                    </h3>
                </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($ud['tasks'] as $t)
                            ...
                            <tr style="background-color:{{ $rowColor }};">
                                <td style="padding:4px 3px; text-align:center; border:1px solid #dee2e6; font-weight:600; color:#495057; font-size:9px;">{{ $i }}</td>
                                <td style="padding:4px 3px; text-align:left; border:1px solid #dee2e6; color:#212529; font-size:9px;">{{ $customer }}</td>
                                <td style="padding:4px 3px; text-align:left; border:1px solid #dee2e6; color:#212529; font-size:9px;">{!! $taskNameHtml !!}</td>
                                <td style="padding:4px 3px; text-align:center; border:1px solid #dee2e6;">{!! $typeBadge !!}</td>
                                <td style="padding:4px 3px; text-align:center; border:1px solid #dee2e6;">{!! $priority !!}</td>
                                <td style="padding:4px 3px; text-align:center; border:1px solid #dee2e6;">{!! $status !!}</td>
                                <td style="padding:4px 3px; text-align:center; border:1px solid #dee2e6; font-size:8px; color:#6c757d;">{{ $createdDate }}</td>
                                <td style="padding:4px 3px; text-align:left; border:1px solid #dee2e6; font-size:8px; color:#495057;">{{ $createdBy }}</td>
                                <td style="padding:4px 3px; text-align:left; border:1px solid #dee2e6; font-size:9px; color:#495057;">{{ $lrShort }}</td>
                                <td style="padding:4px 3px; text-align:center; border:1px solid #dee2e6; font-size:8px; color:#64748b;">{{ $remarkDate }}</td>
                            </tr>
                            @php $i++; @endphp
                        @endforeach
                    </tbody>
    ...
    <div style="background-color:#f8f9fa; padding:15px; text-align:center; font-size:11px; color:#6c757d; border-top:1px solid #dee2e6;">
        Report generated on {{ $payload['timeDisplay'] }}<br>
        © {{ date('Y') }} Workorio. All rights reserved.<br>
        Total Pending Tasks: {{ $payload['totalTasks'] }} across {{ $payload['totalUsers'] }} users.
    </div>
