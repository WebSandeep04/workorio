...
@section('title', 'Invoice Followup')
@section('page_title', 'Invoice Followup')
...
<div class="container-fluid px-2">
  ...
  <div class="summary-cards">
    <div class="summary-card card-1">
      ...
      <div class="summary-card-content">
        ...
        <div class="summary-card-value" id="invoice_amount">₹{{ number_format($invoice->amount ?? 0, 2) }}</div>
      </div>
    </div>
    <div class="summary-card card-2">
      ...
      <div class="summary-card-content">
        ...
        <div class="summary-card-value" id="total_paid">₹{{ number_format($totalPaid ?? 0, 2) }}</div>
      </div>
    </div>
    <div class="summary-card card-3">
      ...
      <div class="summary-card-content">
        ...
        <div class="summary-card-value" id="remaining_amount">₹{{ number_format($remainingAmount ?? 0, 2) }}</div>
      </div>
    </div>
  </div>
  ...
  <div class="invoice-info-card" id="invoiceInfoCard">
    ...
    <div class="invoice-details" id="invoiceDetails">
      <span><strong>Invoice No:</strong> <span id="invoice_number">{{ $invoice->invoice_number ?? 'N/A' }}</span></span>
      <span><strong>Due Date:</strong> <span id="invoice_due_date">{{ $invoice->due_date ? \Carbon\Carbon::parse($invoice->due_date)->format('Y-m-d') : 'N/A' }}</span></span>
      <span><strong>Status:</strong> <span id="invoice_status">{{ ucfirst($invoice->status ?? 'N/A') }}</span></span>
    </div>
  </div>
    ...
    <div class="modal-content" style="border-radius: 0px;">
      ...
      <form id="addFollowupForm">
        <div class="modal-body" style="padding: 1.5rem;">
          ...
          <input type="hidden" id="followup_invoice_id" name="invoice_id" value="{{ $invoiceId }}">
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input type="date" class="form-control form-control-modern" id="followup_payment_date" name="payment_date" value="{{ date('Y-m-d') }}" required>
            </div>
            <div class="col-md-6">
...
function loadFollowups(page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoice-followup.data", ":id") }}?page=${page}`.replace(':id', invoiceId),
        type: 'GET',
...
function updateSummaryCards() {
    ...
    $.ajax({
        url: `{{ route("invoice-followup.data", ":id") }}?per_page=1000`.replace(':id', invoiceId),
        type: 'GET',
...
$(document).on('click', '.edit-followup-btn', function() {
    ...
    // Fetch followup data
    $.ajax({
        url: `{{ route("invoice-followup.get", [":invoiceId", ":id"]) }}`.replace(':invoiceId', invoiceId).replace(':id', followupId),
        type: 'GET',
...
$('#addFollowupForm').on('submit', function(e) {
    ...
    const url = isEdit 
        ? `{{ route("invoice-followup.update", [":invoiceId", ":id"]) }}`.replace(':invoiceId', invoiceId).replace(':id', followupId)
        : `{{ route("invoice-followup.store", ":id") }}`.replace(':id', invoiceId);
...
function searchFollowups(page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoice-followup.data", ":id") }}?search=${encodeURIComponent(search)}&page=${page}`.replace(':id', invoiceId),
        type: 'GET',
        success: function (response) {
...
function loadDateFilteredFollowups(from_date = '', to_date = '', followup_from_date = '', followup_to_date = '', page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoice-followup.data", ":id") }}?from_date=${from_date}&to_date=${to_date}&followup_from_date=${followup_from_date}&followup_to_date=${followup_to_date}&page=${page}`.replace(':id', invoiceId),
        type: 'GET',
        success: function (response) {
            let data = response.data;
            let html = '';
