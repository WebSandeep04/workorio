...
@section('page_title', 'Invoices - Lead Invoices')
          ...
          <div class="mb-3">
            ...
            <div class="row g-3">
              ...
              <div class="col-md-6">
                ...
                <input type="date" class="form-control form-control-modern" id="lumpsum_payment_date" name="payment_date" value="{{ date('Y-m-d') }}" required>
              </div>
              <div class="col-md-6">
...
let defaultSalesRecordId = @json($defaultSalesRecordId);
let leadDataCache = {};
...
function loadInvoices(page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoices.index") }}?customer_id=${customerId}&page=${page}`,
        type: 'GET',
...
function fetchLeadData(salesRecordId, callback) {
    ...
    $.ajax({
        url: `{{ route("payment-followup.lead", ":id") }}`.replace(':id', salesRecordId),
        type: 'GET',
...
$(document).on('click', '.edit-invoice-btn', function() {
    ...
    // Fetch invoice data
    $.ajax({
        url: `{{ route("invoices.get", ":id") }}`.replace(':id', invoiceId),
        type: 'GET',
...
$('#addInvoiceForm').on('submit', function(e) {
    ...
    const url = isEdit 
        ? `{{ route("invoices.update", ":id") }}`.replace(':id', invoiceId)
...
function searchInvoices(page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoices.index") }}?customer_id=${customerId}&search=${encodeURIComponent(search)}&page=${page}`,
        type: 'GET',
        success: function (response) {
...
function loadFilteredInvoices(page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoices.index") }}?customer_id=${customerId}&status=${$('#invoice_status').val()}&page=${page}`,
        type: 'GET',
        success: function (response) {
            let data = response.data;
            let html = '';
...
function loadDateFilteredInvoices(from_date = '', to_date = '', page = 1) {
    ...
    $.ajax({
        url: `{{ route("invoices.index") }}?customer_id=${customerId}&from_date=${from_date}&to_date=${to_date}&page=${page}`,
        type: 'GET',
        success: function (response) {
            let data = response.data;
            let html = '';
...
$('#lumpsum_amount').on('input', function() {
    ...
    if (amount > 0 && customerIdVal) {
        ...
        $.ajax({
            url: `{{ route("invoices.index") }}?customer_id=${customerIdVal}&per_page=1000`,
            type: 'GET',
            success: function(response) {
...
$('#payLumpsumForm').on('submit', function(e) {
    ...
    $.ajax({
        url: '{{ route("payment-followup.pay-lumpsum") }}',
        type: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
