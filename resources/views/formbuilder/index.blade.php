...
@section('page_title', 'Form Builder - Lead Forms')
...
<div class="container-fluid px-2">
  ...
  <div class="summary-cards mb-3">
    <div class="summary-card card-1" style="max-width: 250px;">
      <div class="summary-card-icon icon-violet">
        ...
        <img src="{{ asset('img/icons/pending.png') }}" alt="Total Forms" onerror="this.onerror=null;this.src='{{ asset('img/icons/file.png') }}';">
      </div>
      <div class="summary-card-content">
        <div class="summary-card-label">TOTAL FORMS</div>
        <div class="summary-card-value">{{ $forms->total() }}</div>
      </div>
    </div>
  </div>
  ...
  <div class="table-search mb-2">
    ...
    <a href="{{ route('formbuilder.create') }}" class="table-search-btn">
          ...
          <tbody>
            ...
            <tr>
              <td class="text-start ps-3">
                <div class="fw-semibold text-dark">{{ $form->name }}</div>
              </td>
              <td>
                <span class="badge badge-field-count">{{ count($form->fields ?? []) }} fields</span>
              </td>
              <td>{{ $form->created_at->format('d M Y') }}</td>
              <td>
                {{ $form->updated_at->format('d M Y') }} <small class="text-muted">{{ $form->updated_at->format('h:i A') }}</small>
              </td>
              <td>
                <a href="{{ route('formbuilder.view', $form->id) }}" class="action-btn" title="View & Embed" data-bs-toggle="tooltip">
                    <i class="bi bi-code-slash"></i>
                </a>
                <a href="{{ route('formbuilder.config', $form->id) }}" class="action-btn" title="Database Config" data-bs-toggle="tooltip">
                    <i class="bi bi-database-gear"></i>
                </a>
                <a href="{{ route('formbuilder.edit', $form->id) }}" class="action-btn" title="Edit Form" data-bs-toggle="tooltip">
                    <i class="bi bi-pencil"></i>
                </a>
                <button type="button" class="action-btn btn-delete web-ripple" onclick="deleteForm({{ $form->id }}, '{{ $form->name }}')" title="Delete" data-bs-toggle="tooltip">
  ...
  <div class="mt-2 text-muted small ms-1">
       Page {{ $forms->currentPage() }} of {{ $forms->lastPage() }} • Showing {{ $forms->firstItem() }}-{{ $forms->lastItem() }} of {{ $forms->total() }}
  </div>

  <div class="mt-2 d-flex justify-content-center">
    <nav>
        <ul class="pagination pagination-sm mb-0">
             <li class="page-item {{ $forms->onFirstPage() ? 'disabled' : '' }}">
                 <a class="page-link" href="{{ $forms->previousPageUrl() }}"><i class="bi bi-chevron-left"></i> Previous</a>
             </li>
             <li class="page-item active">
                 <span class="page-link">{{ $forms->currentPage() }} / {{ $forms->lastPage() }}</span>
             </li>
             <li class="page-item {{ $forms->hasMorePages() ? '' : 'disabled' }}">
                 <a class="page-link" href="{{ $forms->nextPageUrl() }}">Next <i class="bi bi-chevron-right"></i></a>
             </li>
        </ul>
     </nav>
  </div>
...
async function deleteForm(id, name) {
    ...
    try {
        const res = await fetch(`{{ url('/form-builder') }}/${id}`, {
            method: 'DELETE',
            headers: {
