...
@section('page_title', isset($form) ? 'Edit Form: ' . $form->name : 'Create New Form')
    ...
    <div class="card fb-card shadow-sm">
        <div class="card-header d-flex justify-content-between align-items-center fb-header">
            <h5 class="mb-0 text-white">
                ...
                {{ isset($form) ? 'Edit Form' : 'Create New Form' }}
            </h5>
            <div>
                <button class="btn btn-primary btn-sm" id="fb-save-form">
                    <i class="bi bi-save me-1"></i>{{ isset($form) ? 'Update Form' : 'Save Form' }}
                </button>
            </div>
        </div>
        <div class="card-body">
                        ...
                        <div class="panel-body">
                            <div class="mb-3">
                                ...
                                <input type="text" id="fb-form-name" class="form-control form-control-sm"
                                       placeholder="Enter form name" value="{{ isset($form) ? $form->name : '' }}">
                            </div>
...
@push('scripts')
<script>
const isEditMode = {{ isset($form) ? 'true' : 'false' }};
const formId = {{ isset($form) ? $form->id : 'null' }};
const savedFields = @json(isset($form) ? $form->fields : []);
...
async function saveForm(){
    ...
    try{
        const url = isEditMode 
            ? `{{ url('/form-builder') }}/${formId}`
            : `{{ route('formbuilder.store') }}`;
        ...
        window.location.href = `{{ route('formbuilder.index') }}`;
    }catch(e){
        alert(e.message);
    }
}
